/* tracker-bus.c generated by valac 0.11.4, the Vala compiler
...
void tracker_bus_steroids_update_array_begin (TrackerBusSteroids* self, GUnixInputStream* sparql_stream, gint priority, GCancellable* cancellable, GAsyncReadyCallback callback, void* callback_target);
...
void tracker_bus_steroids_update_array_begin (TrackerBusSteroids* self, GUnixInputStream* sparql_stream, gint priority, GCancellable* cancellable, GAsyncReadyCallback callback, void* callback_target) {
	g_return_if_fail (sparql_stream != NULL);
	if (priority <= G_PRIORITY_DEFAULT) {
		tracker_bus_steroids_update_array (self, sparql_stream, cancellable, callback, callback_target);
	} else {
		tracker_bus_steroids_batch_update_array (self, sparql_stream, cancellable, callback, callback_target);
	}
}
...
TrackerBusConnection* tracker_bus_connection_construct (GType object_type, GError** error) {
	...
	TrackerBusSteroids* _tmp3_ = NULL;
	TrackerBusSteroids* _tmp4_;
	TrackerBusSteroids* _tmp5_;
	TrackerBusStatistics* _tmp6_ = NULL;
	TrackerBusStatistics* _tmp7_;
	TrackerBusStatistics* _tmp8_;
	...
	tracker_bus_connection_statistics_object = _tmp8_;
...
static void tracker_bus_connection_pipe (TrackerBusConnection* self, GUnixInputStream** input, GUnixOutputStream** output, GError** error) {
	...
	_input = _tmp3_;
	_tmp4_ = (GUnixOutputStream*) g_unix_output_stream_new (pipefd[1], TRUE);
	_tmp5_ = _tmp4_;
...
static gboolean tracker_bus_connection_real_query_async_co (TrackerBusConnectionQueryAsyncData* data) {
	...
	data->_tmp2_ = NULL;
	_g_object_unref0 (data->output);
	data->output = data->_tmp2_;
	data->_tmp3_ = NULL;
	data->_tmp3_ = (GMemoryOutputStream*) g_memory_output_stream_new (NULL, 0, _g_realloc_grealloc_func, _g_free_gdestroy_notify);
	data->mem_stream = data->_tmp3_;
	data->_state_ = 1;
	...
	data->_tmp5_ = NULL;
	data->_tmp5_ = tracker_bus_steroids_query_finish (tracker_bus_connection_steroids_object, data->_data2_->dbus_res, &data->_tmp4_, &data->_inner_error_);
	data->_tmp6_ = data->_tmp5_;
	data->variable_names_length1 = data->_tmp4_;
	data->_variable_names_size_ = data->variable_names_length1;
	data->variable_names = data->_tmp6_;
	if (data->_inner_error_ != NULL) {
		if (((data->_inner_error_->domain == TRACKER_SPARQL_ERROR) || (data->_inner_error_->domain == G_IO_ERROR)) || (data->_inner_error_->domain == G_DBUS_ERROR)) {
			g_simple_async_result_set_from_error (data->_async_result, data->_inner_error_);
			g_error_free (data->_inner_error_);
	...
	data->_tmp7_ = NULL;
	data->_tmp7_ = g_memory_output_stream_steal_data (data->mem_stream);
	data->_tmp8_ = g_memory_output_stream_get_data_size (data->mem_stream);
	data->_tmp9_ = NULL;
	data->_tmp9_ = tracker_bus_fd_cursor_new (data->_tmp7_, data->_tmp8_, data->variable_names, data->variable_names_length1);
	data->result = (TrackerSparqlCursor*) data->_tmp9_;
	data->variable_names = (_vala_array_free (data->variable_names, data->variable_names_length1, (GDestroyNotify) g_free), NULL);
	_g_object_unref0 (data->mem_stream);
	_g_object_unref0 (data->output);
	_g_object_unref0 (data->input);
...
static gboolean tracker_bus_connection_real_update_array_async_co (TrackerBusConnectionUpdateArrayAsyncData* data) {
	...
	tracker_bus_steroids_update_array_begin (tracker_bus_connection_steroids_object, data->input, data->priority, data->cancellable, __lambda4__gasync_ready_callback, data->_data5_);
	data->_tmp2_ = NULL;
	data->_tmp2_ = g_data_output_stream_new ((GOutputStream*) data->output);
	data->data_stream = data->_tmp2_;
	g_data_output_stream_set_byte_order (data->data_stream, G_DATA_STREAM_BYTE_ORDER_HOST_ENDIAN);
	...
	data->_tmp6_ = NULL;
	data->_tmp6_ = g_ptr_array_new_with_free_func (_g_error_free0_);
	data->_result_ = data->_tmp6_;
	if (data->priority <= G_PRIORITY_DEFAULT) {
		data->_tmp7_ = NULL;
		data->_tmp7_ = tracker_bus_steroids_update_array_finish (tracker_bus_connection_steroids_object, data->_data5_->dbus_res, &data->_inner_error_);
		...
		_g_variant_unref0 (data->resultv);
		data->resultv = data->_tmp9_;
	} else {
		data->_tmp10_ = NULL;
		data->_tmp10_ = tracker_bus_steroids_batch_update_array_finish (tracker_bus_connection_steroids_object, data->_data5_->dbus_res, &data->_inner_error_);
		...
		_g_variant_unref0 (data->resultv);
		data->resultv = data->_tmp12_;
	}
	data->_tmp13_ = NULL;
	data->_tmp13_ = g_variant_iter_new (data->resultv);
	data->iter = data->_tmp13_;
	while (TRUE) {
		data->_tmp14_ = g_variant_iter_next (data->iter, "s", &data->code, NULL);
		if (!data->_tmp14_) {
			break;
		}
		data->_tmp15_ = g_variant_iter_next (data->iter, "s", &data->message, NULL);
		if (data->_tmp15_) {
			if (g_strcmp0 (data->code, "") != 0) {
				data->_tmp16_ = g_strcmp0 (data->message, "") != 0;
			} else {
				data->_tmp16_ = FALSE;
			}
			if (data->_tmp16_) {
				data->_tmp17_ = NULL;
				data->_tmp17_ = g_error_new_literal (TRACKER_SPARQL_ERROR, TRACKER_SPARQL_ERROR_INTERNAL, data->message);
				g_ptr_array_add (data->_result_, data->_tmp17_);
			} else {
...
static gboolean tracker_bus_connection_real_update_blank_async_co (TrackerBusConnectionUpdateBlankAsyncData* data) {
	...
	data->_tmp5_ = NULL;
	data->_tmp5_ = tracker_bus_steroids_update_blank_finish (tracker_bus_connection_steroids_object, data->_data7_->dbus_res, &data->_inner_error_);
	data->_tmp6_ = data->_tmp5_;
	if (data->_inner_error_ != NULL) {
		if (((data->_inner_error_->domain == TRACKER_SPARQL_ERROR) || (data->_inner_error_->domain == G_IO_ERROR)) || (data->_inner_error_->domain == G_DBUS_ERROR)) {
			g_simple_async_result_set_from_error (data->_async_result, data->_inner_error_);
...
static TrackerSparqlCursor* tracker_bus_connection_real_statistics (TrackerSparqlConnection* base, GCancellable* cancellable, GError** error) {
	...
	gchar* _tmp11_;
	gchar** _tmp12_;
	TrackerBusArrayCursor* _tmp13_ = NULL;
	GError * _inner_error_ = NULL;
	self = (TrackerBusConnection*) base;
	...
	_tmp4_ = g_new0 (TrackerSparqlValueType, 2);
	_tmp5_ = _tmp4_;
	types_length1 = 2;
	...
	_tmp12_ = results;
	results = NULL;
	_tmp13_ = tracker_bus_array_cursor_new (_tmp12_, results_length1, results_length2, results_length1, results_length2, var_names, var_names_length1, types, types_length1);
	result = (TrackerSparqlCursor*) _tmp13_;
...
static gboolean tracker_bus_connection_real_statistics_async_co (TrackerBusConnectionStatisticsAsyncData* data) {
	...
	data->_tmp4_ = NULL;
	data->_tmp4_ = g_new0 (TrackerSparqlValueType, 2);
	data->_tmp5_ = data->_tmp4_;
	data->types_length1 = 2;
	data->_types_size_ = data->types_length1;
	data->types = data->_tmp5_;
	data->_tmp6_ = NULL;
	data->_tmp6_ = g_new0 (gchar*, 2 + 1);
	data->_tmp7_ = data->_tmp6_;
	data->var_names_length1 = 2;
	data->_var_names_size_ = data->var_names_length1;
	data->var_names = data->_tmp7_;
	data->_tmp8_ = g_strdup ("class");
	data->_tmp9_ = data->_tmp8_;
	_g_free0 (data->var_names[0]);
	data->var_names[0] = data->_tmp9_;
	data->_tmp10_ = g_strdup ("count");
	data->_tmp11_ = data->_tmp10_;
	_g_free0 (data->var_names[1]);
	data->var_names[1] = data->_tmp11_;
	data->types[0] = TRACKER_SPARQL_VALUE_TYPE_STRING;
	data->types[1] = TRACKER_SPARQL_VALUE_TYPE_INTEGER;
	data->_tmp12_ = data->results;
	data->results = NULL;
	data->_tmp13_ = NULL;
	data->_tmp13_ = tracker_bus_array_cursor_new (data->_tmp12_, data->results_length1, data->results_length2, data->results_length1, data->results_length2, data->var_names, data->var_names_length1, data->types, data->types_length1);
	data->result = (TrackerSparqlCursor*) data->_tmp13_;
