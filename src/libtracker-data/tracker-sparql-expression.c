...
static TrackerPropertyType tracker_sparql_expression_translate_primary_expression (TrackerSparqlExpression* self, GString* sql, GError** error) {
	...
	switch (_tmp0_) {
		...
		case TRACKER_SPARQL_TOKEN_TYPE_COALESCE:
		{
			...
			_tmp50_ = tracker_sparql_expression_translate_function (self, sql, TRACKER_SPARQL_EXPRESSION_TRACKER_NS "coalesce", &_inner_error_);
			_result_ = _tmp50_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
					g_propagate_error (error, _inner_error_);
		...
		case TRACKER_SPARQL_TOKEN_TYPE_IF:
		{
			...
			_tmp51_ = tracker_sparql_expression_translate_if_call (self, sql, &_inner_error_);
			_tmp52_ = _tmp51_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
					g_propagate_error (error, _inner_error_);
		...
		case TRACKER_SPARQL_TOKEN_TYPE_SUM:
		{
			...
			_tmp53_ = tracker_sparql_expression_translate_aggregate_expression (self, sql, &_inner_error_);
			_tmp54_ = _tmp53_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
					g_propagate_error (error, _inner_error_);
		...
		case TRACKER_SPARQL_TOKEN_TYPE_AVG:
		{
			...
			_tmp55_ = tracker_sparql_expression_translate_aggregate_expression (self, sql, &_inner_error_);
			_tmp56_ = _tmp55_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
					g_propagate_error (error, _inner_error_);
		...
		case TRACKER_SPARQL_TOKEN_TYPE_MIN:
		{
			...
			_tmp57_ = tracker_sparql_expression_translate_aggregate_expression (self, sql, &_inner_error_);
			_tmp58_ = _tmp57_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
					g_propagate_error (error, _inner_error_);
		...
		case TRACKER_SPARQL_TOKEN_TYPE_MAX:
		{
			...
			_tmp59_ = tracker_sparql_expression_translate_aggregate_expression (self, sql, &_inner_error_);
			_tmp60_ = _tmp59_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
					g_propagate_error (error, _inner_error_);
		...
		case TRACKER_SPARQL_TOKEN_TYPE_GROUP_CONCAT:
		{
			...
			_tmp61_ = tracker_sparql_expression_parse_string_literal (self, NULL, &_inner_error_);
			_tmp62_ = _tmp61_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
					g_propagate_error (error, _inner_error_);
			...
			_tmp63_ = _tmp62_;
			_tmp64_ = tracker_sparql_expression_escape_sql_string_literal (self, _tmp63_);
			_tmp65_ = _tmp64_;
			g_string_append (sql, _tmp65_);
			_g_free0 (_tmp65_);
			_g_free0 (_tmp63_);
			g_string_append (sql, ")");
			tracker_sparql_expression_expect (self, TRACKER_SPARQL_TOKEN_TYPE_CLOSE_PARENS, &_inner_error_);
		...
		case TRACKER_SPARQL_TOKEN_TYPE_PN_PREFIX:
		{
			...
			_tmp70_ = _tmp69_;
			_tmp71_ = tracker_sparql_query_resolve_prefixed_name (self->priv->query, ns, _tmp70_, &_inner_error_);
			_tmp72_ = _tmp71_;
			_g_free0 (_tmp70_);
			_g_free0 (_tmp68_);
			uri = _tmp72_;
			...
			_tmp73_ = tracker_sparql_expression_translate_uri_expression (self, sql, uri, &_inner_error_);
			_tmp74_ = _tmp73_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
					g_propagate_error (error, _inner_error_);
		...
		case TRACKER_SPARQL_TOKEN_TYPE_COLON:
		{
			...
			_tmp75_ = tracker_sparql_expression_get_last_string (self, 0);
			_tmp76_ = _tmp75_;
			_tmp77_ = string_substring (_tmp76_, (glong) 1, (glong) (-1));
			_tmp78_ = _tmp77_;
			_tmp79_ = tracker_sparql_query_resolve_prefixed_name (self->priv->query, "", _tmp78_, &_inner_error_);
			_tmp80_ = _tmp79_;
			_g_free0 (_tmp78_);
			_g_free0 (_tmp76_);
			uri = _tmp80_;
			...
			_tmp81_ = tracker_sparql_expression_translate_uri_expression (self, sql, uri, &_inner_error_);
			_tmp82_ = _tmp81_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
					g_propagate_error (error, _inner_error_);
		...
		{
			GError* _tmp83_ = NULL;
			_tmp83_ = tracker_sparql_expression_get_error (self, "expected primary expression");
...
static TrackerPropertyType tracker_sparql_expression_process_relational_expression (TrackerSparqlExpression* self, GString* sql, glong begin, guint n_bindings, TrackerPropertyType op1type, const gchar* operator, GError** error) {
	...
	if (_tmp1_) {
		...
		if (op1type == TRACKER_PROPERTY_TYPE_DATE) {
			_tmp7_ = op2type == TRACKER_PROPERTY_TYPE_STRING;
		} else {
			_tmp7_ = FALSE;
		}
		if (_tmp7_) {
			...
			if (op1type == TRACKER_PROPERTY_TYPE_STRING) {
				_tmp8_ = op2type == TRACKER_PROPERTY_TYPE_DATE;
			} else {
				_tmp8_ = FALSE;
			}
			_tmp6_ = _tmp8_;
		...
		if (_tmp6_) {
			guint _tmp9_;
			_tmp9_ = g_list_length (self->priv->query->bindings);
			if (_tmp9_ == (n_bindings + 1)) {
				GList* _tmp10_ = NULL;
				_tmp10_ = g_list_last (self->priv->query->bindings);
				((TrackerSparqlDataBinding*) ((TrackerSparqlLiteralBinding*) _tmp10_->data))->data_type = TRACKER_PROPERTY_TYPE_DATE;
			}
		}
	}
	result = TRACKER_PROPERTY_TYPE_BOOLEAN;
