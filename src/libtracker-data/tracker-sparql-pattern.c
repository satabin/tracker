/* tracker-sparql-pattern.c generated by valac 0.11.5, the Vala compiler
...
TrackerSparqlContext* tracker_sparql_pattern_translate_group_graph_pattern (TrackerSparqlPattern* self, GString* sql, GError** error) {
	...
	while (TRUE) {
		gboolean _tmp13_;
		gboolean _tmp14_;
		_tmp13_ = tracker_sparql_pattern_accept (self, TRACKER_SPARQL_TOKEN_TYPE_OPTIONAL, &_inner_error_);
		_tmp14_ = _tmp13_;
		if (_inner_error_ != NULL) {
			if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
				g_propagate_error (error, _inner_error_);
		...
		if (_tmp14_) {
			...
			if (!in_group_graph_pattern) {
				gboolean _tmp16_;
				_tmp16_ = tracker_sparql_pattern_is_simple_optional (self);
				_tmp15_ = _tmp16_;
			} else {
				_tmp15_ = FALSE;
			}
			if (_tmp15_) {
				...
				_tmp18_ = tracker_sparql_pattern_parse_var_or_term (self, sql, &_tmp17_, &_inner_error_);
				self->priv->current_subject_is_var = _tmp17_;
				_tmp19_ = _tmp18_;
				if (_inner_error_ != NULL) {
					if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
						g_propagate_error (error, _inner_error_);
				...
				_tmp20_ = _tmp19_;
				_g_free0 (self->priv->current_subject);
				self->priv->current_subject = _tmp20_;
				...
				_tmp23_ = tracker_sparql_pattern_translate_group_graph_pattern (self, sql, &_inner_error_);
				_tmp24_ = _tmp23_;
				if (_inner_error_ != NULL) {
					if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
						g_propagate_error (error, _inner_error_);
				...
				_tmp25_ = _tmp24_;
				tracker_sparql_pattern_set_context (self, _tmp25_);
				_tracker_sparql_context_unref0 (_tmp25_);
				g_string_append_printf (sql, ") AS t%d_g", right_index);
				first = TRUE;
				first_common = TRUE;
				_tmp26_ = tracker_sparql_pattern_get_context (self);
				_tmp27_ = g_hash_table_get_keys (_tmp26_->var_set);
				{
					GList* v_collection;
					GList* v_it;
					v_collection = _tmp27_;
					for (v_it = v_collection; v_it != NULL; v_it = v_it->next) {
						TrackerSparqlVariable* v;
						v = (TrackerSparqlVariable*) v_it->data;
						{
							...
							_tmp29_ = g_hash_table_lookup (_tmp28_->parent_context->var_set, v);
							old_state = GPOINTER_TO_INT (_tmp29_);
							if (old_state == 0) {
								TrackerSparqlContext* _tmp30_ = NULL;
								TrackerSparqlVariable* _tmp31_;
								const gchar* _tmp32_ = NULL;
								_tmp30_ = tracker_sparql_pattern_get_context (self);
								_tmp31_ = _g_object_ref0 (v);
								g_hash_table_insert (_tmp30_->parent_context->var_set, _tmp31_, GINT_TO_POINTER ((gint) TRACKER_SPARQL_VARIABLE_STATE_OPTIONAL));
								_tmp32_ = tracker_sparql_variable_get_sql_expression (v);
								g_string_append_printf (select, "t%d_g.%s", right_index, _tmp32_);
							} else {
								if (first_common) {
								...
								if (old_state == TRACKER_SPARQL_VARIABLE_STATE_BOUND) {
									...
									g_string_append_printf (sql, "t%d_g.%s = t%d_g.%s", left_index, _tmp33_, right_index, _tmp34_);
									_tmp35_ = tracker_sparql_variable_get_sql_expression (v);
									g_string_append_printf (select, "t%d_g.%s", left_index, _tmp35_);
								} else {
									if (old_state == TRACKER_SPARQL_VARIABLE_STATE_OPTIONAL) {
										...
										g_string_append_printf (sql, "(t%d_g.%s IS NULL OR t%d_g.%s = t%d_g.%s)", left_index, _tmp36_, left_index, _tmp37_, right_index, _tmp38_);
										...
										g_string_append_printf (select, "COALESCE (t%d_g.%s, t%d_g.%s) AS %s", left_index, _tmp39_, right_index, _tmp40_, _tmp41_);
									}
								}
							}
						}
				...
				_tmp42_ = tracker_sparql_pattern_get_context (self);
				_tmp43_ = g_hash_table_get_keys (_tmp42_->parent_context->var_set);
				{
					GList* v_collection;
					GList* v_it;
					v_collection = _tmp43_;
					for (v_it = v_collection; v_it != NULL; v_it = v_it->next) {
						TrackerSparqlVariable* v;
						v = (TrackerSparqlVariable*) v_it->data;
						{
							...
							_tmp45_ = g_hash_table_lookup (_tmp44_->var_set, v);
							if (GPOINTER_TO_INT (_tmp45_) == 0) {
								...
								g_string_append_printf (select, "t%d_g.%s", left_index, _tmp46_);
							}
						}
					}
					_g_list_free0 (v_collection);
				...
				tracker_sparql_pattern_set_context (self, _tmp47_->parent_context);
			...
			gboolean _tmp48_;
			gboolean _tmp49_;
			_tmp48_ = tracker_sparql_pattern_accept (self, TRACKER_SPARQL_TOKEN_TYPE_GRAPH, &_inner_error_);
			_tmp49_ = _tmp48_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
					g_propagate_error (error, _inner_error_);
			...
			if (_tmp49_) {
				...
				gchar* _tmp56_;
				gchar* _tmp57_;
				_tmp50_ = g_strdup (self->current_graph);
				old_graph = _tmp50_;
				old_graph_is_var = self->priv->current_graph_is_var;
				_tmp52_ = tracker_sparql_pattern_parse_var_or_term (self, sql, &_tmp51_, &_inner_error_);
				self->priv->current_graph_is_var = _tmp51_;
				_tmp53_ = _tmp52_;
				if (_inner_error_ != NULL) {
					if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
						g_propagate_error (error, _inner_error_);
				...
				self->current_graph = _tmp54_;
				if (!in_triples_block) {
					_tmp55_ = !in_group_graph_pattern;
				} else {
					_tmp55_ = FALSE;
				}
				if (_tmp55_) {
				...
				_tmp56_ = g_strdup (old_graph);
				_tmp57_ = _tmp56_;
				_g_free0 (self->current_graph);
				self->current_graph = _tmp57_;
				self->priv->current_graph_is_var = old_graph_is_var;
				_g_free0 (old_graph);
			} else {
				TrackerSparqlTokenType _tmp58_;
				_tmp58_ = tracker_sparql_pattern_current (self);
				if (_tmp58_ == TRACKER_SPARQL_TOKEN_TYPE_OPEN_BRACE) {
					gboolean _tmp59_ = FALSE;
					if (!in_triples_block) {
						_tmp59_ = !in_group_graph_pattern;
					} else {
						_tmp59_ = FALSE;
					}
					if (_tmp59_) {
					...
					TrackerSparqlTokenType _tmp60_;
					_tmp60_ = tracker_sparql_pattern_current (self);
					if (_tmp60_ == TRACKER_SPARQL_TOKEN_TYPE_FILTER) {
						TrackerSourceLocation _tmp61_ = {0};
						TrackerSourceLocation _tmp62_ = {0};
						_tmp62_ = (tracker_sparql_pattern_get_location (self, &_tmp61_), _tmp61_);
						_vala_array_add6 (&filters, &filters_length1, &_filters_size_, &_tmp62_);
	...
	if (!in_triples_block) {
		_tmp63_ = !in_group_graph_pattern;
	} else {
		_tmp63_ = FALSE;
	}
	if (_tmp63_) {
	...
	if (filters_length1 > 0) {
		TrackerSourceLocation _tmp64_ = {0};
		TrackerSourceLocation _tmp65_ = {0};
		TrackerSourceLocation end;
		_tmp65_ = (tracker_sparql_pattern_get_location (self, &_tmp64_), _tmp64_);
		end = _tmp65_;
		{
	...
	tracker_sparql_pattern_set_context (self, _tmp66_->parent_context);
	result = _result_;
	filters = (g_free (filters), NULL);
	return result;
}
