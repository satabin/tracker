...
static GVariant* _variant_new1 (GHashTable* value) {
	...
	gpointer _tmp60_;
	gpointer _tmp61_;
	g_hash_table_iter_init (&_tmp59_, value);
	g_variant_builder_init (&_tmp58_, G_VARIANT_TYPE ("a{ss}"));
	while (g_hash_table_iter_next (&_tmp59_, &_tmp60_, &_tmp61_)) {
		gchar* _key;
		gchar* _value;
		_key = (gchar*) _tmp60_;
		_value = (gchar*) _tmp61_;
		g_variant_builder_add (&_tmp58_, "{?*}", g_variant_new_string (_key), g_variant_new_string (_value));
	}
	return g_variant_ref_sink (g_variant_builder_end (&_tmp58_));
}
...
static void tracker_sparql_query_execute_insert_or_delete (TrackerSparqlQuery* self, GVariantBuilder* update_blank_nodes, GError** error) {
	...
	TrackerSourceLocation _tmp23_ = {0};
	TrackerSourceLocation template_location;
	TrackerSourceLocation _tmp36_ = {0};
	TrackerSourceLocation _tmp37_ = {0};
	TrackerSourceLocation after_where;
	gboolean first;
	GList* _tmp38_ = NULL;
	TrackerDBCursor* _tmp45_ = NULL;
	...
	if (_tmp4_) {
		...
		if (self->priv->current_graph == NULL) {
			gboolean _tmp10_;
			gboolean _tmp11_;
			_tmp10_ = tracker_sparql_query_accept (self, TRACKER_SPARQL_TOKEN_TYPE_INTO, &_inner_error_);
			_tmp11_ = _tmp10_;
			if (_inner_error_ != NULL) {
				g_propagate_error (error, _inner_error_);
				return;
		...
		update_statements = FALSE;
		blank = FALSE;
		_tmp12_ = tracker_sparql_query_accept (self, TRACKER_SPARQL_TOKEN_TYPE_SILENT, &_inner_error_);
		_tmp13_ = _tmp12_;
		if (_inner_error_ != NULL) {
			g_propagate_error (error, _inner_error_);
			return;
		...
		if (self->priv->current_graph == NULL) {
			gboolean _tmp15_;
			gboolean _tmp16_;
			_tmp15_ = tracker_sparql_query_accept (self, TRACKER_SPARQL_TOKEN_TYPE_FROM, &_inner_error_);
			_tmp16_ = _tmp15_;
			if (_inner_error_ != NULL) {
				g_propagate_error (error, _inner_error_);
				return;
	...
	if (self->priv->current_graph == NULL) {
		gboolean _tmp18_;
		gboolean _tmp19_;
		_tmp18_ = tracker_sparql_query_accept (self, TRACKER_SPARQL_TOKEN_TYPE_DATA, &_inner_error_);
		_tmp19_ = _tmp18_;
		if (_inner_error_ != NULL) {
			g_propagate_error (error, _inner_error_);
			return;
	...
	sql = _tmp21_;
	_tmp23_ = (tracker_sparql_query_get_location (self, &_tmp22_), _tmp22_);
	template_location = _tmp23_;
	if (!data) {
		gboolean _tmp24_;
		gboolean _tmp25_;
		...
		_tmp24_ = tracker_sparql_query_accept (self, TRACKER_SPARQL_TOKEN_TYPE_WHERE, &_inner_error_);
		_tmp25_ = _tmp24_;
		if (_inner_error_ != NULL) {
			g_propagate_error (error, _inner_error_);
			_g_string_free0 (sql);
		...
		if (_tmp25_) {
			...
			gchar* _tmp31_;
			_tmp26_ = g_strdup (self->priv->current_graph);
			_tmp27_ = _tmp26_;
			_g_free0 (self->pattern->current_graph);
			self->pattern->current_graph = _tmp27_;
			_tmp28_ = tracker_sparql_pattern_translate_group_graph_pattern (self->pattern, pattern_sql, &_inner_error_);
			_tmp29_ = _tmp28_;
			if (_inner_error_ != NULL) {
				g_propagate_error (error, _inner_error_);
				_g_string_free0 (sql);
			...
			_tmp30_ = _tmp29_;
			_tracker_sparql_context_unref0 (self->context);
			self->context = _tmp30_;
			_tmp31_ = NULL;
			_g_free0 (self->pattern->current_graph);
			self->pattern->current_graph = _tmp31_;
		} else {
		...
		_tmp34_ = tracker_sparql_context_new (self, NULL);
		_tmp35_ = _tmp34_;
		_tracker_sparql_context_unref0 (self->context);
		self->context = _tmp35_;
		g_string_append (pattern_sql, "SELECT 1");
	}
	_tmp37_ = (tracker_sparql_query_get_location (self, &_tmp36_), _tmp36_);
	after_where = _tmp37_;
	g_string_append (sql, "SELECT ");
	first = TRUE;
	_tmp38_ = g_hash_table_get_keys (self->context->var_set);
	{
		...
		for (variable_it = variable_collection; variable_it != NULL; variable_it = variable_it->next) {
			...
			variable = (TrackerSparqlVariable*) variable_it->data;
			{
				...
				if (variable->binding == NULL) {
					...
					_tmp40_ = g_strdup_printf ("use of undefined variable `%s'", _tmp39_);
					_tmp41_ = _tmp40_;
					_tmp42_ = tracker_sparql_query_get_error (self, _tmp41_);
					_inner_error_ = (_tmp43_ = _tmp42_, _g_free0 (_tmp41_), _tmp43_);
					g_propagate_error (error, _inner_error_);
				...
				_tmp44_ = tracker_sparql_variable_get_sql_expression (variable);
				tracker_sparql_expression_append_expression_as_string (sql, _tmp44_, ((TrackerSparqlDataBinding*) variable->binding)->data_type);
			}
		}
		_g_list_free0 (variable_collection);
	}
	...
	g_string_append (sql, pattern_sql->str);
	g_string_append (sql, ")");
	_tmp45_ = tracker_sparql_query_exec_sql_cursor (self, sql->str, NULL, 0, NULL, 0, FALSE, &_inner_error_);
	cursor = _tmp45_;
	if (_inner_error_ != NULL) {
		g_propagate_error (error, _inner_error_);
		_g_string_free0 (sql);
	...
	while (TRUE) {
		...
		_tmp46_ = tracker_sparql_cursor_next ((TrackerSparqlCursor*) cursor, NULL, &_inner_error_);
		_tmp47_ = _tmp46_;
		if (_inner_error_ != NULL) {
			g_propagate_error (error, _inner_error_);
		...
		self->priv->blank_nodes = _tmp49_;
		_tmp50_ = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, g_free);
		var_value_map = _tmp50_;
		var_idx = 0;
		_tmp51_ = g_hash_table_get_keys (self->context->var_set);
		{
			GList* variable_collection;
			GList* variable_it;
			variable_collection = _tmp51_;
			for (variable_it = variable_collection; variable_it != NULL; variable_it = variable_it->next) {
				TrackerSparqlVariable* variable;
				variable = (TrackerSparqlVariable*) variable_it->data;
				{
					...
					gchar* _tmp56_;
					_tmp52_ = tracker_sparql_variable_get_name (variable);
					_tmp53_ = g_strdup (_tmp52_);
					_tmp54_ = var_idx;
					var_idx = _tmp54_ + 1;
					_tmp55_ = tracker_sparql_cursor_get_string ((TrackerSparqlCursor*) cursor, _tmp54_, NULL);
					_tmp56_ = g_strdup (_tmp55_);
					g_hash_table_insert (var_value_map, _tmp53_, _tmp56_);
				}
			}
			_g_list_free0 (variable_collection);
		}
		...
		if (blank) {
			_tmp57_ = update_blank_nodes != NULL;
		} else {
			_tmp57_ = FALSE;
		}
		if (_tmp57_) {
	...
	self->bindings = _tmp62_;
	_tmp63_ = _tracker_sparql_context_ref0 (self->context->parent_context);
	_tmp64_ = _tmp63_;
	_tracker_sparql_context_unref0 (self->context);
