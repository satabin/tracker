/* tracker-sparql-scanner.c generated by valac 0.9.5.4-19a5, the Vala compiler
	...
	if (self->priv->current >= self->priv->end) {
		...
		if (g_ascii_isalpha (self->priv->current[0])) {
			...
			while (TRUE) {
				gboolean _tmp0_ = FALSE;
				if (self->priv->current < self->priv->end) {
					_tmp0_ = tracker_sparql_scanner_is_pn_char (self, self->priv->current[0]);
				} else {
					_tmp0_ = FALSE;
				}
				if (!_tmp0_) {
			...
			type = tracker_sparql_scanner_get_identifier_or_keyword (self, begin, len);
		} else {
				...
				switch (self->priv->current[0]) {
					...
					case ':':
					{
						...
						while (TRUE) {
							gboolean _tmp1_ = FALSE;
							if (self->priv->current < self->priv->end) {
								_tmp1_ = tracker_sparql_scanner_is_pn_local_char (self, self->priv->current[0]);
							} else {
								_tmp1_ = FALSE;
							}
							if (!_tmp1_) {
					...
					case '$':
					{
						...
						while (TRUE) {
							gboolean _tmp2_ = FALSE;
							if (self->priv->current < self->priv->end) {
								_tmp2_ = tracker_sparql_scanner_is_varname_char (self, self->priv->current[0]);
							} else {
								_tmp2_ = FALSE;
							}
							if (!_tmp2_) {
						...
						if (self->priv->current < self->priv->end) {
							...
							while (TRUE) {
								...
								if (self->priv->current[0] >= 0x00) {
									_tmp6_ = self->priv->current[0] < 0x20;
								} else {
									_tmp6_ = FALSE;
								}
								if (_tmp6_) {
					...
					case '"':
					{
						...
						if (self->priv->current < (self->priv->end - 6)) {
							_tmp11_ = begin[1] == begin[0];
						} else {
							_tmp11_ = FALSE;
						}
						if (_tmp11_) {
							_tmp10_ = begin[2] == begin[0];
						} else {
							_tmp10_ = FALSE;
						}
						if (_tmp10_) {
							...
							while (TRUE) {
								...
								if (self->priv->current[0] == begin[0]) {
									_tmp13_ = self->priv->current[1] == begin[0];
								} else {
									_tmp13_ = FALSE;
								}
								if (_tmp13_) {
									_tmp12_ = self->priv->current[2] == begin[0];
								} else {
									_tmp12_ = FALSE;
								}
								if (_tmp12_) {
									...
									if (self->priv->current[0] == '\n') {
										...
										if (self->priv->current[0] <= 0x7f) {
											...
											if (u != ((gunichar) (-1))) {
												...
												_inner_error_ = g_error_new (TRACKER_SPARQL_ERROR, TRACKER_SPARQL_ERROR_PARSE, "%d.%d: invalid UTF-8 character", self->priv->line, self->priv->column + token_length_in_chars);
												{
							...
							if (self->priv->current[0] == begin[0]) {
								_tmp15_ = self->priv->current[1] == begin[0];
							} else {
								_tmp15_ = FALSE;
							}
							if (_tmp15_) {
								_tmp14_ = self->priv->current[2] == begin[0];
							} else {
								_tmp14_ = FALSE;
							}
							if (_tmp14_) {
								...
								_inner_error_ = g_error_new (TRACKER_SPARQL_ERROR, TRACKER_SPARQL_ERROR_PARSE, "%d.%d: syntax error, expected \"\"\"", self->priv->line, self->priv->column + token_length_in_chars);
								{
									if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
										g_propagate_error (error, _inner_error_);
										return 0;
						...
						while (TRUE) {
							...
							if (self->priv->current < self->priv->end) {
								_tmp16_ = self->priv->current[0] != begin[0];
							} else {
								_tmp16_ = FALSE;
							}
							if (!_tmp16_) {
							...
							if (self->priv->current[0] == '\\') {
								...
								switch (self->priv->current[0]) {
									...
									default:
									{
										_inner_error_ = g_error_new (TRACKER_SPARQL_ERROR, TRACKER_SPARQL_ERROR_PARSE, "%d.%d: invalid escape sequence", self->priv->line, self->priv->column + token_length_in_chars);
										{
											if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
												g_propagate_error (error, _inner_error_);
												return 0;
						...
						if (_tmp17_) {
							...
							_inner_error_ = g_error_new (TRACKER_SPARQL_ERROR, TRACKER_SPARQL_ERROR_PARSE, "%d.%d: syntax error, expected %c", self->priv->line, self->priv->column + token_length_in_chars, begin[0]);
							{
								if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
									g_propagate_error (error, _inner_error_);
									return 0;
					...
					case '^':
					{
						...
						type = TRACKER_SPARQL_TOKEN_TYPE_NONE;
						if (self->priv->current < (self->priv->end - 2)) {
							_tmp18_ = self->priv->current[0] == self->priv->current[1];
						} else {
							_tmp18_ = FALSE;
						}
						if (_tmp18_) {
							...
							_inner_error_ = g_error_new (TRACKER_SPARQL_ERROR, TRACKER_SPARQL_ERROR_PARSE, "%d.%d: syntax error, unexpected character", self->priv->line, self->priv->column);
							{
								if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
									g_propagate_error (error, _inner_error_);
									return 0;
					...
					default:
					{
						...
						if (u != ((gunichar) (-1))) {
							...
							_inner_error_ = g_error_new (TRACKER_SPARQL_ERROR, TRACKER_SPARQL_ERROR_PARSE, "%d.%d: invalid UTF-8 character", self->priv->line, self->priv->column);
							{
								if (_inner_error_->domain == TRACKER_SPARQL_ERROR) {
									g_propagate_error (error, _inner_error_);
									return 0;
	...
	keyword_array = keyword;
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				if (g_ascii_toupper (begin[i]) != keyword_array[i]) {
					result = FALSE;
					return result;
				}
			}
