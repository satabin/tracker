/* tracker-miner-flickr.c generated by valac 0.13.1, the Vala compiler
...
static void tracker_miner_flickr_association_status_changed (TrackerMinerFlickr* self, GObject* source, GParamSpec* pspec) {
	...
	if (_tmp0_) {
		guint _tmp1_;
		if (self->priv->pull_timeout_handle != ((guint) 0)) {
			return;
		}
...
static gboolean tracker_miner_flickr_init_pull_co (TrackerMinerFlickrInitPullData* _data_) {
	...
	;
	_data_->_tmp0_ = NULL;
	_data_->_tmp0_ = rest_proxy_new_call (_data_->self->priv->rest);
	_g_object_unref0 (_data_->albums_call);
	_data_->albums_call = _data_->_tmp0_;
	rest_proxy_call_add_param (_data_->albums_call, "method", "flickr.photosets.getList");
	{
		_data_->_tmp1_ = NULL;
		_data_->_tmp1_ = tracker_miner_flickr_run_call (_data_->self, _data_->albums_call, &_data_->_inner_error_);
		_data_->_tmp2_ = _data_->_tmp1_;
		if (_data_->_inner_error_ != NULL) {
			goto __catch2_g_error;
		}
		_rest_xml_node_unref0 (_data_->photosets_node);
		_data_->photosets_node = _data_->_tmp2_;
		tracker_miner_flickr_insert_photosets (_data_->self, _data_->photosets_node);
	}
	goto __finally2;
	__catch2_g_error:
	{
		...
		_data_->_tmp3_ = NULL;
		_data_->_tmp3_ = g_strdup_printf ("Could not get photosets list: %s", _data_->call_error->message);
		_data_->error_message = _data_->_tmp3_;
		g_object_set ((TrackerMiner*) _data_->self, "status", _data_->error_message, NULL);
		g_warning ("tracker-miner-flickr.vala:188: %s", _data_->error_message);
...
static gboolean tracker_miner_flickr_writeback_photo_co (TrackerMinerFlickrWritebackPhotoData* _data_) {
	...
	_data_->_tmp1_ = NULL;
	_data_->_tmp1_ = g_hash_table_new_full (g_str_hash, g_str_equal, _g_free0_, _g_free0_);
	_data_->flickr_tags = _data_->_tmp1_;
	_data_->_tmp2_ = NULL;
	_data_->_tmp2_ = g_new0 (gchar*, 0 + 1);
	_data_->tags_to_add = _data_->_tmp2_;
	_data_->tags_to_add_length1 = 0;
	_data_->_tags_to_add_size_ = _data_->tags_to_add_length1;
	_data_->_tmp3_ = NULL;
	_data_->_tmp3_ = g_new0 (gchar*, 0 + 1);
	_data_->tags_to_remove = _data_->_tmp3_;
	_data_->tags_to_remove_length1 = 0;
	_data_->_tags_to_remove_size_ = _data_->tags_to_remove_length1;
	{
		_data_->_tmp4_ = NULL;
		_data_->_tmp4_ = tracker_miner_get_connection ((TrackerMiner*) _data_->self);
		_data_->_tmp5_ = _g_object_ref0 (_data_->_tmp4_);
		_data_->c = _data_->_tmp5_;
		_data_->_tmp6_ = NULL;
		_data_->_tmp6_ = g_strdup_printf ("select tracker:uri (%d) {}", _data_->id);
		_data_->_tmp7_ = _data_->_tmp6_;
		_data_->_state_ = 1;
		tracker_sparql_connection_query_async (_data_->c, _data_->_tmp7_, NULL, tracker_miner_flickr_writeback_photo_ready, _data_);
		return FALSE;
		_state_1:
		_data_->_tmp8_ = NULL;
		_data_->_tmp8_ = tracker_sparql_connection_query_finish (_data_->c, _data_->_res_, &_data_->_inner_error_);
		...
		_data_->cursor = _data_->_tmp10_;
		_data_->_tmp11_ = tracker_sparql_cursor_next (_data_->cursor, NULL, &_data_->_inner_error_);
		_data_->_tmp12_ = _data_->_tmp11_;
		if (_data_->_inner_error_ != NULL) {
			_g_object_unref0 (_data_->c);
			goto __catch9_g_error;
		}
		g_return_if_fail (_data_->_tmp12_ != TRUE);
		_data_->_tmp13_ = NULL;
		_data_->_tmp13_ = tracker_sparql_cursor_get_string (_data_->cursor, 0, NULL);
		_data_->_tmp14_ = g_strdup (_data_->_tmp13_);
		_g_free0 (_data_->photo_urn);
		_data_->photo_urn = _data_->_tmp14_;
		_data_->_tmp15_ = NULL;
		_data_->_tmp15_ = g_strdup_printf ("select ?photo_id ?tag where { <%s> nie:dataSource <%s> ;" "nao:identifier ?photo_id ;" "nao:hasTag ?t ." "?t nao:prefLabel ?tag }", _data_->photo_urn, TRACKER_MINER_FLICKR_DATASOURCE_URN);
		_data_->s = _data_->_tmp15_;
		_data_->_state_ = 2;
		tracker_sparql_connection_query_async (_data_->c, _data_->s, NULL, tracker_miner_flickr_writeback_photo_ready, _data_);
		return FALSE;
		_state_2:
		_data_->_tmp16_ = NULL;
		_data_->_tmp16_ = tracker_sparql_connection_query_finish (_data_->c, _data_->_res_, &_data_->_inner_error_);
...
"photo %d: %s", _data_->id, _data_->query_error->message);
	...
	}
	{
		...
		while (TRUE) {
			_data_->_tmp18_ = tracker_sparql_cursor_next (_data_->cursor, NULL, &_data_->_inner_error_);
			...
			if (_data_->photo_id == NULL) {
				_data_->_tmp20_ = NULL;
				_data_->_tmp20_ = tracker_sparql_cursor_get_string (_data_->cursor, 0, NULL);
				_data_->_tmp21_ = g_strdup (_data_->_tmp20_);
				_g_free0 (_data_->photo_id);
				_data_->photo_id = _data_->_tmp21_;
			}
			_data_->_tmp22_ = NULL;
			_data_->_tmp22_ = tracker_sparql_cursor_get_string (_data_->cursor, 1, NULL);
			_data_->_tmp23_ = g_strdup (_data_->_tmp22_);
			_vala_array_add1 (&_data_->local_tags, &_data_->local_tags_length1, &_data_->_local_tags_size_, _data_->_tmp23_);
		}
		_data_->_tmp24_ = NULL;
		_data_->_tmp24_ = tracker_sparql_cursor_get_string (_data_->cursor, 0, NULL);
		_data_->_tmp25_ = g_strdup (_data_->_tmp24_);
		_g_free0 (_data_->photo_id);
		_data_->photo_id = _data_->_tmp25_;
	}
	goto __finally10;
	__catch10_g_error:
	{
...
"to ids: %s", _data_->cursor_error->message);
	...
	_data_->_tmp26_ = NULL;
	_data_->_tmp26_ = rest_proxy_new_call (_data_->self->priv->rest);
	_g_object_unref0 (_data_->tag_call);
	_data_->tag_call = _data_->_tmp26_;
	rest_proxy_call_add_params (_data_->tag_call, "method", "flickr.tags.getListPhoto", "photo_id", _data_->photo_id, NULL);
	{
		_data_->_tmp27_ = NULL;
		_data_->_tmp27_ = tracker_miner_flickr_run_call (_data_->self, _data_->tag_call, &_data_->_inner_error_);
		_data_->_tmp28_ = _data_->_tmp27_;
		if (_data_->_inner_error_ != NULL) {
			goto __catch11_g_error;
		}
		_rest_xml_node_unref0 (_data_->root_node);
		_data_->root_node = _data_->_tmp28_;
	}
	goto __finally11;
	__catch11_g_error:
	{
		...
		_data_->_inner_error_ = NULL;
		_data_->_tmp29_ = NULL;
		_data_->_tmp29_ = g_strdup_printf ("Couldn't get tags for photo %s: %s", _data_->photo_urn, _data_->get_tags_error->message);
		_data_->error_message = _data_->_tmp29_;
		g_object_set ((TrackerMiner*) _data_->self, "status", _data_->error_message, NULL);
		g_warning ("tracker-miner-flickr.vala:576: %s", _data_->error_message);
		_g_free0 (_data_->error_message);
	...
	_data_->_tmp30_ = NULL;
	_data_->_tmp30_ = rest_xml_node_find (_data_->root_node, "tags");
	_data_->_tmp31_ = NULL;
	_data_->_tmp31_ = rest_xml_node_find (_data_->_tmp30_, "tag");
	_data_->_tmp32_ = _rest_xml_node_ref0 (_data_->_tmp31_);
	_rest_xml_node_unref0 (_data_->tag_node);
	_data_->tag_node = _data_->_tmp32_;
	while (TRUE) {
		if (!(_data_->tag_node != NULL)) {
			break;
		}
		_data_->_tmp33_ = NULL;
		_data_->_tmp33_ = rest_xml_node_get_attr (_data_->tag_node, "raw");
		_data_->_tmp34_ = g_strdup (_data_->_tmp33_);
		_data_->_tmp35_ = NULL;
		_data_->_tmp35_ = rest_xml_node_get_attr (_data_->tag_node, "id");
		_data_->_tmp36_ = g_strdup (_data_->_tmp35_);
		g_hash_table_insert (_data_->flickr_tags, _data_->_tmp34_, _data_->_tmp36_);
		_data_->_tmp37_ = _rest_xml_node_ref0 (_data_->tag_node->next);
		_rest_xml_node_unref0 (_data_->tag_node);
		_data_->tag_node = _data_->_tmp37_;
	}
	{
		_data_->local_tag_collection_length1 = 0;
		_data_->_local_tag_collection_size_ = 0;
		_data_->local_tag_collection = _data_->local_tags;
		_data_->local_tag_collection_length1 = _data_->local_tags_length1;
		for (_data_->local_tag_it = 0; _data_->local_tag_it < _data_->local_tags_length1; _data_->local_tag_it = _data_->local_tag_it + 1) {
			_data_->_tmp38_ = g_strdup (_data_->local_tag_collection[_data_->local_tag_it]);
			_data_->local_tag = _data_->_tmp38_;
			{
				_data_->_tmp39_ = NULL;
				_data_->_tmp39_ = g_hash_table_lookup (_data_->flickr_tags, _data_->local_tag);
				if (((const gchar*) _data_->_tmp39_) == NULL) {
					_data_->_tmp40_ = g_strdup (_data_->local_tag);
					_vala_array_add2 (&_data_->tags_to_add, &_data_->tags_to_add_length1, &_data_->_tags_to_add_size_, _data_->_tmp40_);
				}
	...
	_data_->_tmp41_ = NULL;
	_data_->_tmp41_ = g_hash_table_get_keys (_data_->flickr_tags);
	{
		_data_->flickr_tag_collection = _data_->_tmp41_;
		...
		for (_data_->flickr_tag_it = _data_->flickr_tag_collection; _data_->flickr_tag_it != NULL; _data_->flickr_tag_it = _data_->flickr_tag_it->next) {
			...
			{
				_data_->_tmp42_ = tracker_miner_flickr_array_search_str (_data_->self, _data_->flickr_tag, _data_->local_tags, _data_->local_tags_length1);
				if (_data_->_tmp42_ == (-1)) {
					_data_->_tmp43_ = NULL;
					_data_->_tmp43_ = g_hash_table_lookup (_data_->flickr_tags, _data_->flickr_tag);
					_data_->_tmp44_ = g_strdup ((const gchar*) _data_->_tmp43_);
					_vala_array_add3 (&_data_->tags_to_remove, &_data_->tags_to_remove_length1, &_data_->_tags_to_remove_size_, _data_->_tmp44_);
				}
			}
		}
		_g_list_free0 (_data_->flickr_tag_collection);
	}
	tracker_miner_flickr_add_tags (_data_->self, _data_->photo_id, _data_->tags_to_add, _data_->tags_to_add_length1);
	{
		_data_->tag_collection_length1 = 0;
		_data_->_tag_collection_size_ = 0;
		_data_->tag_collection = _data_->tags_to_remove;
		_data_->tag_collection_length1 = _data_->tags_to_remove_length1;
		for (_data_->tag_it = 0; _data_->tag_it < _data_->tags_to_remove_length1; _data_->tag_it = _data_->tag_it + 1) {
			_data_->_tmp45_ = g_strdup (_data_->tag_collection[_data_->tag_it]);
			_data_->tag = _data_->_tmp45_;
			{
				tracker_miner_flickr_remove_tag (_data_->self, _data_->tag);
				_g_free0 (_data_->tag);
			}
		}
	}
