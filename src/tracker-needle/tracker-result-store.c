/* tracker-result-store.c generated by valac 0.11.4, the Vala compiler
...
static gboolean tracker_result_store_load_operation_co (TrackerResultStoreLoadOperationData* data) {
	...
	data->_state_ = 1;
	tracker_query_perform_async (data->query, (*data->op->node->query).type, (*data->op->node->query).args, (*data->op->node->query).args_length1, data->cancellable, tracker_result_store_load_operation_ready, data);
	return FALSE;
	_state_1:
	data->_tmp2_ = NULL;
	data->_tmp2_ = tracker_query_perform_finish (data->query, data->_res_, &data->_inner_error_);
	...
	data->cursor = data->_tmp4_;
	{
		...
		while (TRUE) {
			...
			return FALSE;
			_state_2:
			data->_tmp6_ = tracker_sparql_cursor_next_finish (data->cursor, data->_res_, &data->_inner_error_);
			...
			__catch7_g_error:
			{
				...
				g_warning ("tracker-result-store.vala:103: Could not fetch row: %s\n", data->ge->message);
			...
			data->_result_ = &data->op->node->results[data->i];
			{
				...
				while (TRUE) {
					...
					if (data->j == (data->self->priv->n_columns - 1)) {
						data->_tmp9_ = NULL;
						data->_tmp9_ = tracker_sparql_cursor_get_string (data->cursor, data->j, NULL);
						data->_tmp10_ = NULL;
						data->_tmp10_ = g_markup_escape_text (data->_tmp9_, (gssize) (-1));
	...
	__catch6_g_io_error:
	{
		...
		data->_inner_error_ = NULL;
		g_warning ("tracker-result-store.vala:134: Could not load items: %s\n", data->ie->message);
	...
	if (data->_tmp20_ > 0) {
		...
		data->next_to_start = data->_tmp24_;
		g_ptr_array_remove (data->self->priv->delayed_operations, data->next_to_start);
		data->_tmp25_ = _g_object_ref0 (data->next_to_start);
		g_ptr_array_add (data->self->priv->running_operations, data->_tmp25_);
		tracker_result_store_load_operation (data->self, data->next_to_start, data->cancellable, NULL, NULL);
		_g_object_unref0 (data->next_to_start);
	} else {
		data->_tmp26_ = g_ptr_array_get_length (data->self->priv->running_operations);
		if (data->_tmp26_ == 0) {
...
static void tracker_result_store_add_operation (TrackerResultStore* self, TrackerResultStoreCategoryNode* cat, gint offset) {
	...
	TrackerResultStoreOperation* op;
	TrackerResultStoreOperation* old;
	...
	op->node = _tmp2_;
	op->offset = offset;
	_tmp3_ = tracker_result_store_find_operation (self, self->priv->running_operations, cat, offset);
	if (_tmp3_ != NULL) {
		_g_object_unref0 (old);
		_g_object_unref0 (op);
		return;
	}
	_tmp4_ = tracker_result_store_find_operation (self, self->priv->delayed_operations, cat, offset);
	_tmp5_ = _tmp4_;
	_g_object_unref0 (old);
	old = _tmp5_;
	if (old != NULL) {
...
static gboolean tracker_result_store_load_category_co (TrackerResultStoreLoadCategoryData* data) {
	...
	tracker_query_set_criteria (data->query, data->self->priv->_search_term);
	data->_state_ = 1;
	tracker_query_get_count_async (data->query, (*data->query_data).type, data->cancellable, tracker_result_store_load_category_ready, data);
	return FALSE;
	_state_1:
	...
	if (data->count != 0) {
		...
		}
		{
			...
			while (TRUE) {
				...
				data->_tmp12_ = NULL;
				data->_tmp12_ = g_new0 (gchar*, data->self->priv->n_columns + 1);
				...
				(*data->res)._values_size_ = (*data->res).values_length1;
				(*data->res).values = data->_tmp13_;
				data->iter.user_data2 = data->res;
				data->_tmp14_ = NULL;
				data->_tmp14_ = GINT_TO_POINTER (data->i);
				data->iter.user_data3 = data->_tmp14_;
				data->_tmp15_ = NULL;
				data->_tmp15_ = gtk_tree_model_get_path ((GtkTreeModel*) data->self, &data->iter);
				data->_tmp16_ = data->_tmp15_;
				_gtk_tree_path_free0 (data->path);
		...
		if (data->self->priv->queries_length1 > 1) {
			...
			data->_tmp17_ = NULL;
			data->_tmp17_ = gtk_tree_model_get_path ((GtkTreeModel*) data->self, &data->iter);
	...
	data->_tmp19_ = g_ptr_array_get_length (data->self->priv->running_operations);
	if (data->_tmp19_ == 0) {
		tracker_result_store_set_active (data->self, FALSE);
	}
	if (data->_state_ == 0) {
...
static void tracker_result_store_clear_results (TrackerResultStore* self) {
	...
	while (TRUE) {
		...
		if (self->priv->queries_length1 > 1) {
			...
			_tmp9_ = gtk_tree_model_get_path ((GtkTreeModel*) self, &iter);
			_tmp10_ = _tmp9_;
			_gtk_tree_path_free0 (path);
			path = _tmp10_;
			gtk_tree_model_row_deleted ((GtkTreeModel*) self, path);
...
static gboolean tracker_result_store_real_get_iter (GtkTreeModel* base, GtkTreeIter* iter, GtkTreePath* path) {
	...
	if (self->priv->queries_length1 > 1) {
		...
		_tmp6_ = g_ptr_array_get_length (self->priv->categories);
		if (_tmp6_ == 0) {
			_iter.stamp = 0;
			result = FALSE;
			_tracker_result_store_category_node_unref0 (cat);
		...
		_tmp7_ = g_ptr_array_index (self->priv->categories, (guint) 0);
		_tmp8_ = _tracker_result_store_category_node_ref0 ((TrackerResultStoreCategoryNode*) _tmp7_);
		_tmp9_ = _tmp8_;
		_tracker_result_store_category_node_unref0 (cat);
		cat = _tmp9_;
	}
	...
	_iter.user_data = cat;
	_tmp10_ = gtk_tree_path_get_depth (path);
	if (_tmp10_ == (i + 1)) {
		void* _tmp11_ = NULL;
		...
		_tmp11_ = GINT_TO_POINTER (indices[i]);
		_iter.user_data3 = _tmp11_;
	}
	result = TRUE;
	_tracker_result_store_category_node_unref0 (cat);
	if (iter) {
...
static gboolean tracker_result_store_fetch_thumbnail_co (TrackerResultStoreFetchThumbnailData* data) {
	...
	_state_1:
	data->_tmp2_ = NULL;
	data->_tmp2_ = g_file_query_info_finish (data->file, data->_res_, &data->_inner_error_);
	data->_tmp3_ = data->_tmp2_;
	if (data->_inner_error_ != NULL) {
	...
	__catch9_g_error:
	{
		data->ie = data->_inner_error_;
		data->_inner_error_ = NULL;
		g_warning ("tracker-result-store.vala:476: Could not get thumbnail: %s", data->ie->message);
		_g_error_free0 (data->ie);
	...
	data->_tmp5_ = NULL;
	data->_tmp5_ = g_file_info_get_attribute_byte_string (data->info, "thumbnail::path");
	data->_tmp6_ = g_strdup (data->_tmp5_);
	data->_tmp7_ = data->_tmp6_;
	_g_free0 (data->thumb_path);
	data->thumb_path = data->_tmp7_;
	if (data->thumb_path != NULL) {
		data->_tmp8_ = NULL;
		data->_tmp8_ = gdk_pixbuf_new_from_file_at_size (data->thumb_path, data->self->priv->_icon_size, data->self->priv->_icon_size, &data->_inner_error_);
		...
		data->_tmp10_ = data->_tmp9_;
		_g_object_unref0 (data->pixbuf);
		data->pixbuf = data->_tmp10_;
	} else {
		data->_tmp11_ = NULL;
		data->_tmp11_ = g_file_info_get_attribute_object (data->info, "standard::icon");
		data->_tmp12_ = _g_object_ref0 (G_ICON (data->_tmp11_));
		data->_tmp13_ = data->_tmp12_;
		_g_object_unref0 (data->icon);
		data->icon = data->_tmp13_;
		if (data->icon == NULL) {
			_g_object_unref0 (data->pixbuf);
			_g_free0 (data->thumb_path);
			_gtk_icon_info_free0 (data->icon_info);
		...
		data->_tmp14_ = NULL;
		data->_tmp14_ = gdk_screen_get_default ();
		data->_tmp15_ = NULL;
		data->_tmp15_ = gtk_icon_theme_get_for_screen (data->_tmp14_);
		data->_tmp16_ = _g_object_ref0 (data->_tmp15_);
		data->theme = data->_tmp16_;
		data->_tmp17_ = NULL;
		data->_tmp17_ = gtk_icon_theme_lookup_by_gicon (data->theme, data->icon, data->self->priv->_icon_size, 0);
		data->_tmp18_ = data->_tmp17_;
		_gtk_icon_info_free0 (data->icon_info);
		data->icon_info = data->_tmp18_;
		...
		data->_tmp19_ = NULL;
		data->_tmp19_ = gtk_icon_info_load_icon (data->icon_info, &data->_inner_error_);
	...
	__catch10_g_error:
	{
		...
		data->_inner_error_ = NULL;
		g_warning ("tracker-result-store.vala:502: Could not get icon pixbuf: %s\n", data->e->message);
	...
	if (data->pixbuf != NULL) {
		data->_tmp22_ = _g_object_ref0 (data->pixbuf);
		data->_tmp23_ = data->_tmp22_;
		_g_object_unref0 ((*data->_result_).pixbuf);
		(*data->_result_).pixbuf = data->_tmp23_;
		data->_tmp24_ = NULL;
		data->_tmp24_ = gtk_tree_model_get_path ((GtkTreeModel*) data->self, &data->iter);
		data->_tmp25_ = data->_tmp24_;
		_gtk_tree_path_free0 (data->path);
		data->path = data->_tmp25_;
		gtk_tree_model_row_changed ((GtkTreeModel*) data->self, data->path, &data->iter);
	}
	_g_object_unref0 (data->pixbuf);
	_g_free0 (data->thumb_path);
...
static void tracker_result_store_real_get_value (GtkTreeModel* base, GtkTreeIter* iter, gint column, GValue* value) {
	...
	_tracker_result_store_category_node_unref0 (cat);
	cat = _tmp1_;
	_tmp2_ = gtk_tree_model_get_column_type ((GtkTreeModel*) self, column);
	g_value_init (&_value, _tmp2_);
	...
	if ((*iter).user_data2 == NULL) {
		if (column == self->priv->n_columns) {
			...
			GdkPixbuf* _tmp4_;
			pixbuf = NULL;
			_tmp3_ = _g_object_ref0 (cat->pixbuf);
			_tmp4_ = _tmp3_;
			...
			if (pixbuf == NULL) {
				...
				_tmp6_ = gtk_icon_theme_get_for_screen (_tmp5_);
				...
				switch (cat->type) {
					case TRACKER_QUERY_TYPE_APPLICATIONS:
					{
						...
						GdkPixbuf* _tmp9_;
						_tmp8_ = tracker_pixbuf_new_from_name (theme, "package-x-generic", size);
						_tmp9_ = _tmp8_;
						_g_object_unref0 (pixbuf);
						pixbuf = _tmp9_;
						break;
					...
					case TRACKER_QUERY_TYPE_MUSIC:
					{
						...
						GdkPixbuf* _tmp11_;
						_tmp10_ = tracker_pixbuf_new_from_name (theme, "audio-x-generic", size);
						_tmp11_ = _tmp10_;
						_g_object_unref0 (pixbuf);
						pixbuf = _tmp11_;
						break;
					...
					case TRACKER_QUERY_TYPE_IMAGES:
					{
						...
						GdkPixbuf* _tmp13_;
						_tmp12_ = tracker_pixbuf_new_from_name (theme, "image-x-generic", size);
						_tmp13_ = _tmp12_;
						_g_object_unref0 (pixbuf);
						pixbuf = _tmp13_;
						break;
					...
					case TRACKER_QUERY_TYPE_VIDEOS:
					{
						...
						GdkPixbuf* _tmp15_;
						_tmp14_ = tracker_pixbuf_new_from_name (theme, "video-x-generic", size);
						_tmp15_ = _tmp14_;
						_g_object_unref0 (pixbuf);
						pixbuf = _tmp15_;
						break;
					...
					case TRACKER_QUERY_TYPE_DOCUMENTS:
					{
						...
						GdkPixbuf* _tmp17_;
						_tmp16_ = tracker_pixbuf_new_from_name (theme, "x-office-presentation", size);
						_tmp17_ = _tmp16_;
						_g_object_unref0 (pixbuf);
						pixbuf = _tmp17_;
						break;
					...
					case TRACKER_QUERY_TYPE_MAIL:
					{
						...
						GdkPixbuf* _tmp19_;
						_tmp18_ = tracker_pixbuf_new_from_name (theme, "emblem-mail", size);
						_tmp19_ = _tmp18_;
						_g_object_unref0 (pixbuf);
						pixbuf = _tmp19_;
						break;
					...
					case TRACKER_QUERY_TYPE_FOLDERS:
					{
						...
						GdkPixbuf* _tmp21_;
						_tmp20_ = tracker_pixbuf_new_from_name (theme, "folder", size);
						_tmp21_ = _tmp20_;
						_g_object_unref0 (pixbuf);
						pixbuf = _tmp21_;
						break;
...
static gboolean tracker_result_store_real_iter_children (GtkTreeModel* base, GtkTreeIter* iter, GtkTreeIter* parent) {
	...
	if (parent == NULL) {
		...
		if (self->priv->queries_length1 > 1) {
			...
			_iter.user_data = (TrackerResultStoreCategoryNode*) _tmp5_;
			_iter.user_data2 = &cat->results[0];
			_tmp6_ = GINT_TO_POINTER (0);
			_iter.user_data3 = _tmp6_;
			result = TRUE;
			_tracker_result_store_category_node_unref0 (cat);
			if (iter) {
				*iter = _iter;
	...
	_tmp7_ = _tracker_result_store_category_node_ref0 (TRACKER_RESULT_STORE_CATEGORY_NODE ((*parent).user_data));
	_tmp8_ = _tmp7_;
	_tracker_result_store_category_node_unref0 (cat);
	cat = _tmp8_;
	if (cat->results_length1 <= 0) {
...
static gboolean tracker_result_store_real_iter_next (GtkTreeModel* base, GtkTreeIter* iter) {
	...
	if ((*iter).user_data2 == NULL) {
		...
		gconstpointer _tmp5_ = NULL;
		_tmp2_ = tracker_result_store_find_nth_category_index (self, cat, 1);
		i = _tmp2_;
		if (i < 0) {
			_tmp3_ = TRUE;
		} else {
		...
		(*iter).stamp = self->priv->timestamp;
		_tmp5_ = g_ptr_array_index (self->priv->categories, (guint) i);
		(*iter).user_data = (TrackerResultStoreCategoryNode*) _tmp5_;
		result = TRUE;
		_tracker_result_store_category_node_unref0 (cat);
		return result;
	} else {
...
static gboolean tracker_result_store_real_iter_nth_child (GtkTreeModel* base, GtkTreeIter* iter, GtkTreeIter* parent, gint n) {
	...
	if (parent != NULL) {
		...
		_tmp2_ = GINT_TO_POINTER (n);
		_iter.user_data3 = _tmp2_;
		result = TRUE;
		_tracker_result_store_category_node_unref0 (cat);
		if (iter) {
			*iter = _iter;
		...
		if (self->priv->queries_length1 > 1) {
			...
			_tmp3_ = tracker_result_store_find_nth_category_index (self, NULL, n);
			index = _tmp3_;
			if (index < 0) {
				_tmp4_ = TRUE;
			} else {
		...
		_tmp6_ = g_ptr_array_index (self->priv->categories, (guint) index);
...
void tracker_result_store_add_query (TrackerResultStore* self, TrackerQueryType type, ...) {
	...
	TrackerResultStoreQueryData _tmp6_ = {0};
	TrackerResultStoreQueryData _tmp7_ = {0};
	TrackerResultStoreQueryData _tmp8_;
	gchar** _tmp9_;
	gchar** _tmp10_;
	gchar** _tmp11_;
	TrackerResultStoreQueryData _tmp12_;
	TrackerResultStoreQueryData _tmp13_ = {0};
	g_return_if_fail (self != NULL);
	va_start (l, type);
	str = NULL;
	_tmp0_ = NULL;
	...
	if (args_length1 != self->priv->n_columns) {
		g_warning ("tracker-result-store.vala:798: Arguments and number of columns doesn't" \
" match");
	...
	memset (&_tmp6_, 0, sizeof (TrackerResultStoreQueryData));
	_tmp7_ = _tmp6_;
	_tmp8_ = _tmp7_;
	tracker_result_store_query_data_destroy (&query_data);
	query_data = _tmp8_;
	query_data.type = type;
	_tmp9_ = (_tmp10_ = args, (_tmp10_ == NULL) ? ((gpointer) _tmp10_) : _vala_array_dup3 (_tmp10_, args_length1));
	_tmp11_ = _tmp9_;
	...
	query_data.args = _tmp11_;
	_tmp12_ = (tracker_result_store_query_data_copy (&query_data, &_tmp13_), _tmp13_);
	_vala_array_add3 (&self->priv->queries, &self->priv->queries_length1, &self->priv->_queries_size_, &_tmp12_);
	tracker_result_store_query_data_destroy (&query_data);
	args = (_vala_array_free (args, args_length1, (GDestroyNotify) g_free), NULL);
	_g_free0 (str);
	va_end (l);
...
void tracker_result_store_set_search_term (TrackerResultStore* self, const gchar* value) {
	...
	_tmp6_ = _tmp5_;
	_g_ptr_array_unref0 (self->priv->categories);
	self->priv->categories = _tmp6_;
	_tmp7_ = g_ptr_array_new_with_free_func (_g_object_unref0_);
	_tmp8_ = _tmp7_;
	_g_ptr_array_unref0 (self->priv->running_operations);
	self->priv->running_operations = _tmp8_;
	_tmp9_ = g_ptr_array_new_with_free_func (_g_object_unref0_);
	_tmp10_ = _tmp9_;
	_g_ptr_array_unref0 (self->priv->delayed_operations);
	self->priv->delayed_operations = _tmp10_;
	self->priv->timestamp++;
	{
		gboolean _tmp11_;
		i = 0;
