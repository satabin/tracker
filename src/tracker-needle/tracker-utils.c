/* tracker-utils.c generated by valac 0.11.5, the Vala compiler
...
GdkPixbuf* tracker_pixbuf_new_from_file (GtkIconTheme* theme, const gchar* filename, gint size, gboolean is_image) {
	...
	file = _tmp0_;
	_tmp2_ = _g_object_ref0 ((_tmp1_ = NULL, GDK_IS_PIXBUF (_tmp1_) ? ((GdkPixbuf*) _tmp1_) : NULL));
	pixbuf = _tmp2_;
	if (is_image) {
		gchar* _tmp3_ = NULL;
		gchar* _tmp4_;
		...
		__catch9_g_error:
		{
			...
			e = _inner_error_;
			_inner_error_ = NULL;
			_tmp9_ = g_strconcat ("Error loading icon pixbuf: ", e->message, NULL);
	...
	if (_tmp10_) {
		...
		if (file_info != NULL) {
			...
			if (G_IS_FILE_ICON (icon)) {
				...
				_tmp19_ = (_tmp18_ = _tmp17_, _g_free0 (_tmp16_), _tmp18_);
				if (_inner_error_ != NULL) {
				...
				if (G_IS_THEMED_ICON (icon)) {
					...
					GdkPixbuf* _tmp24_;
					GdkPixbuf* _tmp25_;
					_tmp22_ = _tmp21_ = g_themed_icon_get_names (G_THEMED_ICON (icon));
					_tmp23_ = gtk_icon_theme_load_icon (theme, _tmp22_[0], size, GTK_ICON_LOOKUP_USE_BUILTIN, &_inner_error_);
					_tmp24_ = _tmp23_;
					if (_inner_error_ != NULL) {
						goto __catch11_g_error;
					}
			...
			__catch11_g_error:
			{
				...
				e = _inner_error_;
				_inner_error_ = NULL;
				_tmp26_ = g_strconcat ("Error loading icon pixbuf: ", e->message, NULL);
				g_warning ("tracker-utils.vala:146: %s", _tmp26_);
				_g_free0 (_tmp26_);
				_g_error_free0 (e);
			}
			__finally11:
		...
		__catch10_g_error:
		{
			...
			e = _inner_error_;
			_inner_error_ = NULL;
			_tmp27_ = g_strconcat ("Error looking up file for pixbuf: ", e->message, NULL);
...
GdkPixbuf* tracker_pixbuf_new_from_name (GtkIconTheme* theme, const gchar* name, gint size) {
	...
	_tmp1_ = _g_object_ref0 ((_tmp0_ = NULL, GDK_IS_PIXBUF (_tmp0_) ? ((GdkPixbuf*) _tmp0_) : NULL));
	pixbuf = _tmp1_;
