/* tracker-preferences.c generated by valac 0.12.1, the Vala compiler
...
void togglebutton_home_toggled_cb (GtkToggleButton* source) {
	...
	_tmp7_ = gtk_toggle_button_get_active (source);
	if (!_tmp7_) {
		gboolean _tmp8_;
		_tmp8_ = model_contains ((GtkTreeModel*) liststore_index_recursively, HOME_STRING);
		_tmp6_ = _tmp8_;
	} else {
		_tmp6_ = FALSE;
	}
	if (_tmp6_) {
		...
		GtkTreeIter _tmp9_ = {0};
		gboolean _tmp10_;
		_tmp10_ = gtk_tree_model_get_iter_first ((GtkTreeModel*) liststore_index_recursively, &_tmp9_);
		iter = _tmp9_;
		valid = _tmp10_;
		while (TRUE) {
			GValue value = {0};
			...
			gtk_tree_model_get_value ((GtkTreeModel*) liststore_index_recursively, &iter, 0, &_tmp11_);
			G_IS_VALUE (&value) ? (g_value_unset (&value), NULL) : NULL;
			value = _tmp11_;
			_tmp12_ = g_value_get_string (&value);
			if (g_strcmp0 (_tmp12_, HOME_STRING) == 0) {
				GtkTreeIter _tmp13_ = {0};
				gboolean _tmp14_;
				gtk_list_store_remove (liststore_index_recursively, &iter);
				_tmp14_ = gtk_tree_model_get_iter_first ((GtkTreeModel*) liststore_index_recursively, &_tmp13_);
				iter = _tmp13_;
				valid = _tmp14_;
			} else {
				gboolean _tmp15_;
				_tmp15_ = gtk_tree_model_iter_next ((GtkTreeModel*) liststore_index_recursively, &iter);
				valid = _tmp15_;
			}
			G_IS_VALUE (&value) ? (g_value_unset (&value), NULL) : NULL;
		}
