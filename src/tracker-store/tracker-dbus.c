/* tracker-dbus.c generated by valac 0.12.0.69-6c49, the Vala compiler
...
static gboolean tracker_dbus_dbus_register_service (const gchar* name) {
	...
	_tmp2_ = g_dbus_connection_call_sync (tracker_dbus_connection, "org.freedesktop.DBus", "/org/freedesktop/DBus", "org.freedesktop.DBus", "RequestName", _tmp1_, (const GVariantType*) "(u)", 0, -1, NULL, &_inner_error_);
	_tmp3_ = _tmp2_;
...
TrackerStatus* tracker_dbus_register_notifier (void) {
	...
	_g_object_unref0 (tracker_dbus_notifier);
	tracker_dbus_notifier = _tmp0_;
	if (tracker_dbus_notifier == NULL) {
		g_critical ("Could not create TrackerStatus object to register");
		result = NULL;
		return result;
	}
	_tmp1_ = tracker_dbus_register_object (TRACKER_TYPE_STATUS, (GBoxedCopyFunc) g_object_ref, g_object_unref, tracker_dbus_connection, tracker_dbus_notifier, TRACKER_STATUS_PATH);
	tracker_dbus_notifier_id = _tmp1_;
	_tmp2_ = _g_object_ref0 (tracker_dbus_notifier);
	result = _tmp2_;
	return result;
}
...
gboolean tracker_dbus_register_objects (void) {
	...
	guint _tmp1_;
	TrackerResources* _tmp2_ = NULL;
	guint _tmp3_;
	guint _tmp4_;
	TrackerSteroids* _tmp5_ = NULL;
	guint _tmp6_;
	if (tracker_dbus_connection == NULL) {
		g_critical ("D-Bus support must be initialized before registering objects!");
		result = FALSE;
	...
	tracker_dbus_statistics = _tmp0_;
	if (tracker_dbus_statistics == NULL) {
		g_critical ("Could not create TrackerStatistics object to register");
		result = FALSE;
		return result;
	}
	_tmp1_ = tracker_dbus_register_object (TRACKER_TYPE_STATISTICS, (GBoxedCopyFunc) g_object_ref, g_object_unref, tracker_dbus_connection, tracker_dbus_statistics, TRACKER_STATISTICS_PATH);
	tracker_dbus_statistics_id = _tmp1_;
	_tmp2_ = tracker_resources_new (tracker_dbus_connection);
	_g_object_unref0 (tracker_dbus_resources);
	tracker_dbus_resources = _tmp2_;
	if (tracker_dbus_resources == NULL) {
		g_critical ("Could not create TrackerResources object to register");
		result = FALSE;
		return result;
	}
	_tmp3_ = g_dbus_connection_signal_subscribe (tracker_dbus_connection, "org.freedesktop.DBus", "org.freedesktop.DBus", "NameOwnerChanged", "/org/freedesktop/DBus", NULL, 0, _tracker_dbus_name_owner_changed_cb_gd_bus_signal_callback, NULL, NULL);
	tracker_dbus_name_owner_changed_id = _tmp3_;
	_tmp4_ = tracker_dbus_register_object (TRACKER_TYPE_RESOURCES, (GBoxedCopyFunc) g_object_ref, g_object_unref, tracker_dbus_connection, tracker_dbus_resources, TRACKER_RESOURCES_PATH);
	tracker_dbus_resources_id = _tmp4_;
	_tmp5_ = tracker_steroids_new ();
	_g_object_unref0 (tracker_dbus_steroids);
	tracker_dbus_steroids = _tmp5_;
	if (tracker_dbus_steroids == NULL) {
		g_critical ("Could not create TrackerSteroids object to register");
		result = FALSE;
		return result;
	}
	_tmp6_ = tracker_dbus_register_object (TRACKER_TYPE_STEROIDS, (GBoxedCopyFunc) g_object_ref, g_object_unref, tracker_dbus_connection, tracker_dbus_steroids, TRACKER_STEROIDS_PATH);
	tracker_dbus_steroids_id = _tmp6_;
	if (tracker_dbus_backup == NULL) {
		TrackerBackup* _tmp7_ = NULL;
		guint _tmp8_;
		_tmp7_ = tracker_backup_new ();
		_g_object_unref0 (tracker_dbus_backup);
		tracker_dbus_backup = _tmp7_;
		if (tracker_dbus_backup == NULL) {
			g_critical ("Could not create TrackerBackup object to register");
			result = FALSE;
			return result;
		}
		_tmp8_ = tracker_dbus_register_object (TRACKER_TYPE_BACKUP, (GBoxedCopyFunc) g_object_ref, g_object_unref, tracker_dbus_connection, tracker_dbus_backup, TRACKER_BACKUP_PATH);
		tracker_dbus_backup_id = _tmp8_;
	}
	result = TRUE;
	return result;
}
