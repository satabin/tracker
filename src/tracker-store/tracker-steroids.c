/* tracker-steroids.c generated by valac 0.12.0, the Vala compiler
...
static gboolean tracker_steroids_update_array_co (TrackerSteroidsUpdateArrayData* data) {
	...
	data->_tmp3_ = NULL;
	data->_tmp3_ = g_string_new ("");
	data->combined_query = data->_tmp3_;
	data->_tmp4_ = NULL;
	data->_tmp4_ = g_new0 (gchar*, data->query_count + 1);
	...
	data->_query_array_size_ = data->query_count;
	{
		...
		while (TRUE) {
			...
			if (data->_inner_error_ != NULL) {
				...
				_g_string_free0 (data->combined_query);
				_g_object_unref0 (data->data_input_stream);
				goto __catch15_g_error;
			}
			data->_tmp7_ = NULL;
			...
			g_string_append (data->combined_query, data->query_array[data->i]);
		}
	}
	_g_object_unref0 (data->data_input_stream);
	data->data_input_stream = NULL;
	data->input_stream = NULL;
	data->_tmp10_ = NULL;
	data->_tmp10_ = g_variant_builder_new ((const GVariantType*) "as");
	data->builder = data->_tmp10_;
	data->_state_ = 1;
	tracker_store_sparql_update (data->combined_query->str, TRACKER_STORE_PRIORITY_LOW, (const gchar*) data->sender, tracker_steroids_update_array_ready, data);
	return FALSE;
	_state_1:
	tracker_store_sparql_update_finish (data->_res_, &data->_inner_error_);
	if (data->_inner_error_ != NULL) {
	...
	tracker_dbus_request_end (data->request, NULL);
	data->_tmp14_ = NULL;
	data->_tmp14_ = g_variant_builder_end (data->builder);
	data->result = g_variant_ref_sink (data->_tmp14_);
	_g_variant_builder_unref0 (data->builder);
	data->query_array = (_vala_array_free (data->query_array, data->query_array_length1, (GDestroyNotify) g_free), NULL);
	_g_string_free0 (data->combined_query);
	_g_object_unref0 (data->data_input_stream);
	...
	__catch15_g_error:
	{
		...
		tracker_dbus_request_end (data->request, data->e);
		if (data->e->domain == TRACKER_SPARQL_ERROR) {
			data->_tmp15_ = _g_error_copy0 (data->e);
			data->_inner_error_ = data->_tmp15_;
			_g_error_free0 (data->e);
			goto __finally15;
		} else {
			data->_tmp16_ = NULL;
			data->_tmp16_ = g_error_new_literal (TRACKER_SPARQL_ERROR, TRACKER_SPARQL_ERROR_INTERNAL, data->e->message);
