/* test-class-signal-performance.c generated by valac 0.11.5, the Vala compiler
...
static void resourcesclass_proxy_g_signal (GDBusProxy* proxy, const gchar* sender_name, const gchar* signal_name, GVariant* parameters) {
	if (strcmp (signal_name, "SubjectsAdded") == 0) {
		_dbus_handle_resourcesclass_subjects_added (proxy, parameters);
	} else if (strcmp (signal_name, "SubjectsChanged") == 0) {
		_dbus_handle_resourcesclass_subjects_changed (proxy, parameters);
	}
}
...
TestApp* test_app_construct (GType object_type) {
	...
	g_signal_connect (test_app_class_object, "subjects-added", (GCallback) _test_app_on_subjects_added_resourcesclass_subjects_added, self);
	g_signal_connect (test_app_class_object, "subjects-changed", (GCallback) _test_app_on_subjects_changed_resourcesclass_subjects_changed, self);
...
static void test_app_on_subjects_changed (TestApp* self, gchar** subjects, int subjects_length1, gchar** preds, int preds_length1) {
	...
	_tmp1_ = g_timer_elapsed (self->priv->t, NULL);
	g_print ("Old class signal count=%d time=%lf\n", self->priv->count, _tmp1_);
}
...
static void test_app_insert_data (TestApp* self) {
	...
	g_timer_start (self->priv->t);
	{
		...
		while (TRUE) {
			...
			_tmp1_ = g_strdup_printf ("INSERT { <%d> a nmm:MusicPiece ; nie:title '%s %d' }", i, title_data, i);
			upqry = _tmp1_;
