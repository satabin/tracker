/* test-default-update.c generated by valac 0.11.5, the Vala compiler
...
static void _lambda0_ (GObject* o, GAsyncResult* res, Block1Data* _data1_) {
	...
	__catch7_g_error:
	{
		...
		e = _inner_error_;
		_inner_error_ = NULL;
		g_warning ("test-default-update.vala:17: Couldn't perform test: %s", e->message);
		_g_error_free0 (e);
	}
	__finally7:
	if (_inner_error_ != NULL) {
...
void test_sync (void) {
	...
	g_print ("Got it %p\n", c);
	g_print ("Creating app with connection\n");
	_tmp3_ = test_app_new (c);
	app = _tmp3_;
	g_print ("Running app\n");
	_tmp4_ = test_app_run (app);
	res = _tmp4_;
	_g_object_unref0 (app);
	_g_object_unref0 (c);
	goto __finally8;
	__catch8_g_error:
	{
		...
		e = _inner_error_;
		_inner_error_ = NULL;
		g_warning ("test-default-update.vala:49: Couldn't perform test: %s", e->message);
		_g_error_free0 (e);
	}
	__finally8:
	if (_inner_error_ != NULL) {
