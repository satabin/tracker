/* tracker-miner-mock.c generated by valac 0.11.5, the Vala compiler
...
void tracker_miner_mock_pause (TrackerMinerMock* self, const gchar* app, const gchar* reason) {
	...
	if (self->priv->_apps_length1 == 0) {
		...
		gchar* _tmp3_;
		_tmp3_ = g_strdup (app);
		_vala_array_add1 (&self->priv->_apps, &self->priv->_apps_length1, &self->priv->__apps_size_, _tmp3_);
	}
	if (self->priv->_reasons_length1 == 0) {
		gchar* _tmp4_;
		gchar** _tmp5_ = NULL;
		gchar** _tmp6_;
		_tmp4_ = g_strdup (reason);
		...
		self->priv->__reasons_size_ = self->priv->_reasons_length1;
		self->priv->_reasons = _tmp6_;
	} else {
		gchar* _tmp7_;
		_tmp7_ = g_strdup (reason);
		_vala_array_add2 (&self->priv->_reasons, &self->priv->_reasons_length1, &self->priv->__reasons_size_, _tmp7_);
	}
	self->is_paused = TRUE;
